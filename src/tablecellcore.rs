...
impl Tablecellcore {
    ...
    pub fn force_as_i8(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_i16(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_i32(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_i64(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_i128(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_u8(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_u16(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_u32(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_u64(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_u128(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_f32(value: &String) -> Result<Self, String> {
    ...
    pub fn force_as_f64(value: &String) -> Result<Self, String> {
...
mod tests {
    ...
    #[test]
    fn test_to_string() {
        let v = Tablecellcore::U8(123);
        assert_eq!(v.to_string(), "123");
        let v = Tablecellcore::U16(12345);
        assert_eq!(v.to_string(), "12345");
        let v = Tablecellcore::U32(123456);
        assert_eq!(v.to_string(), "123456");
        let v = Tablecellcore::U64(123456789);
        assert_eq!(v.to_string(), "123456789");
        let v = Tablecellcore::U128(12345678901234567890);
        assert_eq!(v.to_string(), "12345678901234567890");
        let v = Tablecellcore::I8(-123);
        assert_eq!(v.to_string(), "-123");
        let v = Tablecellcore::I16(-12345);
        assert_eq!(v.to_string(), "-12345");
        let v = Tablecellcore::I32(-123456789);
        assert_eq!(v.to_string(), "-123456789");
        let v = Tablecellcore::I64(-12345678901);
        assert_eq!(v.to_string(), "-12345678901");
        let v = Tablecellcore::I128(-12345678901234567890);
        assert_eq!(v.to_string(), "-12345678901234567890");
        let v = Tablecellcore::F64(123.456);
    ...
    #[test]
    fn test_force_as_i8() {
        let v = Tablecellcore::force_as_i8(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<i8>");
        let v = Tablecellcore::force_as_i8(&"-123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "-123<i8>");
        let v = Tablecellcore::force_as_i8(&"123.456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_i8(&"1234".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_i8(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_i16() {
        let v = Tablecellcore::force_as_i16(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<i16>");
        let v = Tablecellcore::force_as_i16(&"-123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "-123<i16>");
        let v = Tablecellcore::force_as_i16(&"123.456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_i16(&"123456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_i16(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_i32() {
        let v = Tablecellcore::force_as_i32(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<i32>");
        let v = Tablecellcore::force_as_i32(&"-123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "-123<i32>");
        let v = Tablecellcore::force_as_i32(&"123.456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_i32(&"12345678901".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_i32(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_i64() {
        let v = Tablecellcore::force_as_i64(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<i64>");
        let v = Tablecellcore::force_as_i64(&"-123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "-123<i64>");
        let v = Tablecellcore::force_as_i64(&"123.456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_i64(&"123456789012345678901".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_i64(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_i128() {
        let v = Tablecellcore::force_as_i128(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<i128>");
        let v = Tablecellcore::force_as_i128(&"-123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "-123<i128>");
        let v = Tablecellcore::force_as_i128(&"123.456".to_string());
        assert!(v.is_err());
        let v =
            Tablecellcore::force_as_i128(&"1234567890123456789012345678901234567890".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_i128(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_u8() {
        let v = Tablecellcore::force_as_u8(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<u8>");
        let v = Tablecellcore::force_as_u8(&"123.456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u8(&"1234".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u8(&"-123".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u8(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_u16() {
        let v = Tablecellcore::force_as_u16(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<u16>");
        let v = Tablecellcore::force_as_u16(&"123.456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u16(&"123456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u16(&"-123".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u16(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_u32() {
        let v = Tablecellcore::force_as_u32(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<u32>");
        let v = Tablecellcore::force_as_u32(&"123.456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u32(&"12345678901".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u32(&"-123".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u32(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_u64() {
        let v = Tablecellcore::force_as_u64(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<u64>");
        let v = Tablecellcore::force_as_u64(&"123.456".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u64(&"123456789012345678901".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u64(&"-123".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u64(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_u128() {
        let v = Tablecellcore::force_as_u128(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<u128>");
        let v = Tablecellcore::force_as_u128(&"123.456".to_string());
        assert!(v.is_err());
        let v =
            Tablecellcore::force_as_u128(&"1234567890123456789012345678901234567890".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u128(&"-123".to_string());
        assert!(v.is_err());
        let v = Tablecellcore::force_as_u128(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_f32() {
        let v = Tablecellcore::force_as_f32(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<f32>");
        let v = Tablecellcore::force_as_f32(&"123.456".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123.456<f32>");
        let v = Tablecellcore::force_as_f32(&"-123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "-123<f32>");
        let v = Tablecellcore::force_as_f32(&"inf".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "inf<f32>");
        let v = Tablecellcore::force_as_f32(&"NAN".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "NaN<f32>");
        let v = Tablecellcore::force_as_f32(&"-inf".to_string()).unwrap();
        let output = format!("{:?}", v);
        ...
        let output = format!("{:?}", v);
        assert_eq!(output, "NaN<f32>");
        let v = Tablecellcore::force_as_f32(&"value123".to_string());
        assert!(v.is_err());
    }

    #[test]
    fn test_force_as_f64() {
        let v = Tablecellcore::force_as_f64(&"123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123<f64>");
        let v = Tablecellcore::force_as_f64(&"123.456".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "123.456<f64>");
        let v = Tablecellcore::force_as_f64(&"-123".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "-123<f64>");
        let v = Tablecellcore::force_as_f64(&"inf".to_string()).unwrap();
        let output = format!("{:?}", v);
        assert_eq!(output, "inf<f64>");
        let v = Tablecellcore::force_as_f64(&"NAN".to_string()).unwrap();
        let output = format!("{:?}", v);
        ...
        let output = format!("{:?}", v);
        assert_eq!(output, "NaN<f64>");
        let v = Tablecellcore::force_as_f64(&"value123".to_string());
        assert!(v.is_err());
    }
}
